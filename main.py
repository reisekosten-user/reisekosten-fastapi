...
def handle_currency(amount_str: str, currency: str):
    ...
    currency = (currency or "").upper().replace("₹", "INR")
    if currency == "EUR":
    ...
    return {
        ...
        "schema": {
            ...
            "required": [
                ...
                "nights", "currency", "total_amount", "taxes", "line_items", "extras", "confidence", "review_flag"
            ],
        },
    }
...
def schema_restaurant():
    return {
        ...
        "schema": {
            ...
            "required": ["vendor", "location", "document_date", "currency", "total_amount", "tax_amount", "line_items", "extras", "confidence", "review_flag"],
        },
    }
...
def mistral_classify(document_text: str, source_type: str):
    system_prompt = (
        ...
        "Arbeite allgemeingültig, ohne Listen von Städten oder bestimmten Marken. "
        "Unterscheide zuerst in document_group, document_type und document_role. "
        "Für Transport gilt: "
        "Wenn das Dokument mehrere Reiseabschnitte mit Abfahrt/Ankunft, Codes oder Nummern enthält, ist es Transport. "
        "Wenn es Luftverkehr betrifft, aber Abschnitte durch Deutsche Bahn oder andere Bahnunternehmen ausgeführt werden, klassifiziere den betroffenen Transporttyp als Zug in der Detailanalyse. "
        "Wenn es sich um einzelne Fahrten wie Taxi, Uber, Shuttle, Transfer oder Zubringer handelt, ist es sonstiger Transport. "
...
def mistral_extract_by_group(document_text: str, source_type: str, document_group: str):
    if document_group == "transport":
        prompt = (
            ...
            "Klassifiziere jeden Abschnitt als Flug, Zug oder Sonstiger Transport. "
            "Entscheidungsregeln: "
            "Flug, wenn Luftverkehr mit Flugnummer, Flughafen, Airline oder Flugsegmenten vorliegt. "
            "Zug, wenn ausdrücklich Deutsche Bahn, DB, Bahn, Rail, Train oder ein durch Bahn ausgeführter Abschnitt genannt ist. "
            "Auch wenn das Gesamtdokument von einer Airline stammt, darf ein einzelnes Segment Zug sein, wenn es durch Deutsche Bahn ausgeführt wird. "
            "Sonstiger Transport für Taxi, Uber, Shuttle, Transfer, Mietwagen oder ähnliche Einzeltransporte. "
            "Mehrere Segmente desselben Dokuments gehören zusammen. Keine Währungsumrechnung."
        )
        ...
        prompt = (
            ...
            "Arbeite allgemeingültig. Gesamtbetrag, Steuer/MwSt und optionale Positionen erfassen."
        )
        schema = schema_restaurant()
    else:
        prompt = (
            "Extrahiere nur Zeitdaten, Orte und Kontext aus einem unterstützenden Dokument. "
            "Keine Annahmen."
        )
...
def page_shell(title: str, content: str):
    ...
    .topbar {{ background:#12365f; color:white; padding:18px 22px; display:flex; align-items:center; justify-content:space-between; gap:15px; }}
    .topbar-left {{ display:flex; align-items:center; gap:16px; }}
    .topbar img {{ height:66px; display:block; filter:grayscale(1) brightness(1.75) contrast(0.9); }}
    .tool-title {{ font-size:30px; font-weight:700; letter-spacing:.2px; }}
    .tool-status {{ display:flex; gap:10px; margin-left:10px; }}
    .status-box {{ border:1px solid rgba(255,255,255,.25); color:white; padding:8px 10px; border-radius:10px; font-size:13px; min-width:92px; text-align:center; background:rgba(255,255,255,.08); }}
    .version {{ font-size:13px; opacity:.95; }}
    .wrap {{ padding:20px; }}
    .card {{ background:white; padding:20px; border-radius:10px; margin-bottom:20px; }}
    .btn {{ background:#2a6ab1; color:white; padding:10px 12px; border:none; border-radius:6px; text-decoration:none; display:inline-block; }}
    .btn-light {{ background:white; color:#2a6ab1; padding:10px 12px; border:1px solid #b8cbe0; border-radius:6px; text-decoration:none; display:inline-block; }}
    table {{ width:100%; border-collapse:collapse; background:white; }}
    th, td {{ border:1px solid #d9e2ec; padding:8px; text-align:left; vertical-align:top; }}
    th {{ background:#f4f7fb; }}
    .code {{ font-weight:bold; color:#12365f; }}
    .badge-ok {{ color:white; background:#177245; padding:4px 8px; border-radius:999px; font-size:12px; }}
    .badge-warn {{ color:white; background:#b46b00; padding:4px 8px; border-radius:999px; font-size:12px; }}
    .badge-bad {{ color:white; background:#b3261e; padding:4px 8px; border-radius:999px; font-size:12px; }}
    .sub {{ color:#567; font-size:14px; }}
    .columns {{ display:flex; gap:20px; align-items:flex-start; }}
    .col {{ flex:1; }}
    pre {{ white-space:pre-wrap; word-break:break-word; background:#f7f9fc; padding:12px; border-radius:8px; border:1px solid #d9e2ec; }}
    </style></head>
    <body>
        <div class='topbar'>
...
@app.get("/version")
def version():
    return {"version": APP_VERSION, "mistral_configured": bool(MISTRAL_API_KEY), "mistral_model": MISTRAL_MODEL}


@app.get("/init")
def init():
    ensure_schema()
    return {"status": "ok", "version": APP_VERSION, "mistral_configured": bool(MISTRAL_API_KEY)}
...
def trip_detail(trip_code: str):
    try:
        ensure_schema(); conn = get_conn(); cur = conn.cursor()
...
def event_detail(trip_code: str, event_code: str):
    try:
        ...
        for row in rows:
            ...
            """
            (completed_blocks if side == 'completed' else planning_blocks).append(block)
        cur.close(); conn.close()
        return page_shell("Ereignis", f"""
        <div class='card'><h2>Reise {trip_code} / Ereignis {event_code}</h2><p><a class='btn' href='/event-pdf/{trip_code}/{event_code}'>Ereignis-PDF herunterladen</a> <a class='btn-light' href='/trip/{trip_code}'>Zur Reise</a></p></div>
        <div class='columns'><div class='col'><div class='card'><h3>Planung</h3>{''.join(planning_blocks) or 'Keine Daten'}</div></div><div class='col'><div class='card'><h3>Abgeschlossen</h3>{''.join(completed_blocks) or 'Keine Daten'}</div></div></div>
        """)
    except Exception as e:
...
def event_pdf(trip_code: str, event_code: str):
    try:
        ...
        def line(text, step=15):
            ...
            pdf.drawString(40, y, text[:115]); y -= step
        pdf.setTitle(f"Reise_{trip_code}_Ereignis_{event_code}")
        pdf.setFont("Helvetica-Bold", 14); line(f"Reise {trip_code} / Ereignis {event_code}", 22)
        pdf.setFont("Helvetica", 10); line("Zusammenfassung Ereignis", 18); line(" ", 8)
        for row in rows:
...
def download_attachment(attachment_id: int):
    try:
        ...
        s3 = get_s3(); response = s3.get_object(Bucket=S3_BUCKET, Key=storage_key); data = response["Body"].read()
...
def fetch_mails():
    try:
        ensure_schema(); s3 = get_s3()
        mail = imaplib.IMAP4_SSL(IMAP_HOST); mail.login(IMAP_USER, IMAP_PASS); mail.select("INBOX")
...
def analyze_attachments():
    try:
        ...
        rows = cur.fetchall()
        processed = ai_processed = 0
        for attachment_id, trip_code, storage_key, original_filename in rows:
            storage_key = storage_key or ""; original_filename = original_filename or ""
            if not storage_key:
            ...
            status = "ok"
            if text == "NICHT_ANALYSIERBAR": status = "nicht analysierbar"
            elif text.startswith("ERROR:"): status = "analysefehler"
            elif text == "KEIN_TEXT_GEFUNDEN": status = "kein text"
...
def trip_review():
    try:
        ...
        cur.close(); conn.close()
        return page_shell("Reisebewertung", f"<div class='card'><h2>Reisebewertung v6.4</h2><table><tr><th>Code</th><th>Flug</th><th>Hotel</th><th>Transport</th><th>Offene Prüfungen</th><th>Warnungen</th><th>Fehler</th><th>Status</th></tr>{rows_html}</table></div>")
    except Exception as e:
